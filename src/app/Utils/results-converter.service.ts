...
export class ResultsConverterService {
  ...
  obtenerResultadosClas(resultados: string, opciones: Opcion[], totalVotos: number): Map<string, number>[] {
    let votos = new Map<string, number>();
    ...
    for (const string of arrayResultados) {
      ...
      let nVotos = +arrayString.pop();
      votos.set(this.obtenerNombre(arrayString), nVotos);
      votosNo.set(this.obtenerNombre(arrayString), (totalVotos - nVotos));
    }
    for (const opcion of opciones) {
      if (!votosNo.has(opcion.nombre.valueOf())){
        votosNo.set(opcion.nombre.valueOf(), totalVotos);
      }
    }
